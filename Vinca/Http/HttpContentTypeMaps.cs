...
namespace Vinca.Http
{
    ...
    /// <summary>
    /// Try to get content type form file extenions (from http path. example /a/b/test.html-> 'text/html'
    /// This class is thread save and static
    ...
    /// but using concurrent dictionary
    /// </summary>
    public static class HttpContentTypeMaps
    {
        /// <summary>
        /// extension must include dot, example ok:  ".js" | example not ok: "js"
        /// </summary>
        ...
        /// <summary>
        /// if not found returns 'application/octet-stream'
        /// </summary>
        /// <param name="path"></param>
        /// <param name="contentType"></param>
        /// <returns></returns>
        public static string GetFromPathOrFallback(string path, string fallback = "application/octet-stream")
        {
            if (TryGetFromPath(path, out var contentType))
            {
                return contentType;
            }
        ...
        public static bool TryGetFromPath(string path, out string contentType)
        {
            if (!string.IsNullOrWhiteSpace(path) && path.Contains('.'))
            {
                ...
                return TryGet(ext, out contentType);
            }
            else
            {
                contentType = null;
        ...
        private static readonly ConcurrentDictionary<string, string> mappings;

        /// <summary>
        /// 
        /// </summary>
        static HttpContentTypeMaps()
        {
            Dictionary<string, string> mappingsInit = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
                { ".323", "text/h323" },
                { ".3g2", "video/3gpp2" },
                { ".3gp2", "video/3gpp2" },
            ...
            mappings = new ConcurrentDictionary<string, string>();

            foreach (var kv in mappingsInit)
            {
