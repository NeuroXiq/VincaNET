...
namespace DNDocs.Infrastructure.DomainServices
{
    ...
    internal class Git : IGit, IDisposable
    {
        ...
        public void InitInstance(string gitRepoUrl)
        {
            ...
            Validate.AppEx(this.repoOSPath != null, $"Invalid operation: This repo is already initialized, cannot initialize again. Repo: {repoOSPath}");
            Validate.AppEx(string.IsNullOrWhiteSpace(gitRepoUrl), "repoUrl is null or empty");
            ...
            if (!currentThreadLockedOk)
            {
                ...
                Validate.AppEx($"repository already locked: '{repoUrl}'");
            }
            ...
            Validate.AppEx(allDirs.Length != 1,
        ...
        public string RepoOSPath { get { return repoOSPath ?? throw new AppException("Cannot access RepoOSPath because instance not initialized. Call 'InitInstance()' method before using methods"); } }
        ...
        {
            Validate.AppEx(string.IsNullOrWhiteSpace(commit), "commit empty");
